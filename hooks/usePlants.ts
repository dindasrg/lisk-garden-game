  ...
  const fetchPlants = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        // Filter out plants that don't exist or are dead
        const validPlants = userPlants.filter((plant) => plant.isExists && !plant.isDead);
    ...
    async (seedId: bigint = BigInt(0)) => {
      ...
      try {
        ...
        toast({
          title: "Seed planted!",
          description:
            "Your plant has been created successfully. Cost: 0.001 ETH",
        });
        ...
        toast({
          ...
          description:
            err instanceof Error
              ? err.message
              : "Failed to plant seed. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
      ...
      try {
        ...
        toast({
          ...
          description:
            err instanceof Error
              ? err.message
              : "Failed to water plant. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
    ...
    async (plantId: bigint) => {
      ...
      try {
        ...
        if (needsStageUpdate) {
          ...
          await updatePlantStageContract(
            client,
            account as Account,
            plantId,
            address
      ...
      try {
        ...
        toast({
          ...
          description:
            err instanceof Error
              ? err.message
              : "Failed to update plant stage. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
